# soul_engine.py
# pip install numpy uvicorn fastapi pydantic transformers accelerate python-multipart
...
@app.get("/vectors/{concept_name}")
async def download_vector(concept_name: str):
    """Permite que o PC baixe o vetor da Nuvem para Backup local."""
    filepath = os.path.join(VECTOR_DIR, f"{concept_name}.npy")
    if not os.path.exists(filepath):
        raise HTTPException(status_code=404, detail="Vetor não encontrado")
    return FileResponse(filepath, filename=f"{concept_name}.npy")


@app.post("/vectors/upload")
async def upload_vector(file: UploadFile = File(...)):
    """Permite que o PC restaure um vetor para a Nuvem."""
    os.makedirs(VECTOR_DIR, exist_ok=True)
    filepath = os.path.join(VECTOR_DIR, file.filename)
    with open(filepath, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)

    # Carrega na memória RAM da Engine imediatamente
    concept_name = Path(file.filename).stem
    vector_data = np.load(filepath)
    state.vectors[concept_name] = torch.tensor(vector_data, dtype=torch.float32)

    logger.info(f"📥 Vetor restaurado e carregado na RAM: {concept_name}")
    return {"status": "restored", "concept": concept_name}
                ...
                try:
                    ...
                    if request.vector_name and request.vector_name in state.vectors:
                        if request.intensity != 0:
                            ...
                            # MODO 1: VIA DIRETA DA FÍSICA NEURAL (Vetor Puro da RAM do Backend)
                            if request.raw_vector and request.intensity != 0:
                                logger.info(
                                    f"💉 [NEURAL PHYSICS] Injetando VETOR MATEMÁTICO PURO na camada {request.layer_idx} (Intensidade: {request.intensity})")

                                # Converte a lista recebida da API de volta para Tensor
                                # O hook vai se encarregar de mover para a GPU e converter para FP16/BF16
                                steering_vec = torch.tensor(request.raw_vector, dtype=torch.float32)

                                hook_fn = create_steering_hook(steering_vec, request.intensity)
                                hook_handle = target_layer.register_forward_hook(hook_fn)
                                state.active_hooks.append(hook_handle)

                            # MODO 2: FALLBACK LEGADO (Arquivos .npy pré-gravados)
                            elif request.vector_name and request.vector_name in state.vectors:
                                if request.intensity != 0:
                                    logger.info(
                                        f"💉 [LEGACY] Injetando '{request.vector_name}' (Intensidade: {request.intensity}) na camada {request.layer_idx}")
